...
use graphrag::prompt_tune::generator::community_report_summarization::COMMUNITY_SUMMARIZATION_FILENAME;
use graphrag::prompt_tune::generator::entity_summarization_prompt::ENTITY_SUMMARIZATION_FILENAME;
use graphrag::prompt_tune::generator::extract_graph_prompt::EXTRACT_GRAPH_FILENAME;
use graphrag::utils::cli::redact;

use graphrag_cli::index::_logger;
...
pub async fn prompt_tune(
    ...
    if enabled_logging:
        info(format!("Logging enabled at {log_path}"), verbose)
    else:
        info(
            format!("Logging not enabled for config {redact(graph_config.model_dump())}"),
            verbose,
        )
    ...
    if output_path:
        info(format!("Writing prompts to {output_path}"))
        ...
        success(format!("Prompts written to {output_path}"))
    else:
