...
use log::debug;
use polars::prelude::{LazyFrame, concat};
...
use crate::index::operations::extract_graph::typing::{
    Document,
...
pub async fn extract_graph(
    ...
    text_column: str,
    id_column: str,
    strategy: HashMap<String, Box<dyn Any>> | None,
    async_mode: AsyncType = AsyncType.AsyncIO,
    entity_types=DEFAULT_ENTITY_TYPES,
    num_threads: usize = 4,
) -> (LazyFrame, LazyFrame) {
    debug("entity_extract strategy={strategy}");
    if entity_types.is_none():
        entity_types = DEFAULT_ENTITY_TYPES
    let strategy = strategy or {};
    let strategy_exec = _load_strategy(
    ...
    let results = await derive_from_rows(
        text_units,
        run_strategy,
        callbacks,
    ...
    let mut entity_dfs = [];
    let mut relationship_dfs = [];
    for result in results {
        if result:
            entity_dfs.push(LazyFrame(result[0]))
            relationship_dfs.push(LazyFrame(result[1]))
    }

    let entities = _merge_entities(entity_dfs);
    let relationships = _merge_relationships(relationship_dfs);

    (entities, relationships)
}

/// Load strategy method definition.
fn _load_strategy(strategy_type: ExtractEntityStrategyType) -> EntityExtractStrategy {
    match strategy_type {
        ExtractEntityStrategyType.graph_intelligence => {
            use crate::index.operations.extract_graph.graph_intelligence_strategy::run_graph_intelligence;

            run_graph_intelligence
        }
        _ => {
            let msg = format!("Unknown strategy: {strategy_type}");
            raise ValueError(msg)
        }
    }
}

fn _merge_entities<L>(entity_dfs: L) -> LazyFrame
where
    L: AsRef<[LazyFrame]>,
{
    let all_entities = pd.concat(entity_dfs, ignore_index=true);
...
fn _merge_relationships<L>(relationship_dfs: L) -> LazyFrame
where
    L: AsRef<[LazyFrame]>,
{
    let all_relationships = concat(relationship_dfs, ignore_index=false);
