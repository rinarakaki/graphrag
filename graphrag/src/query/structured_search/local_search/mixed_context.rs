...
use crate::query::context_builder::entity_extraction::{
    EntityVectorStoreKey,
    map_query_to_entities,
};
use crate::query::context_builder::local_context::{
    build_covariates_context,
    build_entity_context,
    build_relationship_context,
    get_candidate_context,
};
use crate::query::context_builder::source_context::{
    build_text_unit_context,
    count_relationships,
};
use crate::query::input::retrieval::community_reports::{
    get_candidate_communities,
};
use crate::query::input::retrieval::text_units::get_candidate_text_units;
use crate::query::llm::text_utils::num_tokens;
use crate::query::structured_search::base::LocalContextBuilder;
...
pub struct LocalSearchMixedContext(LocalContextBuilder) {
}
...
impl LocalSearchMixedContext {
    pub fn new(
        ...
        text_units: Vec<TextUnit> | None = None,
        community_reports: Vec<CommunityReport> | None = None,
        relationships: Vec<Relationship> | None = None,
        covariates: dict[str, Vec<Covariate>] | None = None,
        token_encoder: Option<tiktoken_rs::Encoding>,
        embedding_vectorstore_key: str = EntityVectorStoreKey.ID,
    ) {
        if community_reports.is_none():
            community_reports = []
        if relationships.is_none():
            relationships = []
        if covariates.is_none():
            covariates = {}
        if text_units.is_none():
            text_units = []
    ...
    pub fn filter_by_entity_keys(self, entity_keys: Vec<int> | Vec<str>) {
    ...
    pub fn build_context(
        ...
        include_entity_names: Vec<str> | None = None,
        exclude_entity_names: Vec<str> | None = None,
        ...
        **kwargs: HashMap<String, Box<dyn Any>>,
        ...
        // build context
        let mut final_context = Vec::<String>::new();
        let mut final_context_data = HashMap::<String, LazyFrame>::new();
        ...
        if conversation_history {
            ...
            if conversation_history_context.strip() != "" {
                final_context.push(conversation_history_context);
        ...
        let community_tokens = max(int(max_tokens * community_prop), 0);
        ...
        let local_prop = 1 - community_prop - text_unit_prop;
        let local_tokens = max(int(max_tokens * local_prop), 0);
        ...
        if local_context.strip() != "" {
            final_context.push(str(local_context));
            final_context_data.extend(local_context_data);
        }

        let text_unit_tokens = max(int(max_tokens * text_unit_prop), 0);
    ...
    fn _build_community_context(
    ...
    ) -> (String, HashMap<String, LazyFrame>) {
        if len(selected_entities) == 0 or len(self.community_reports) == 0 {
            return ("", {context_name.lower(): LazyFrame()})
        }
        ...
        selected_communities.sort(
            key=|x| (x.attributes["matches"], x.rank),
            reverse=True,
        );
        for community in selected_communities {
        ...
        let (context_text, context_data) = build_community_context(
    ...
    fn _build_text_unit_context(
        ...
        let mut selected_text_units = Vec::new();
        let mut text_unit_ids_set = HashSet::new()

        let mut unit_info_list = Vec::new();
        let relationship_values = list(self.relationships.values());
        ...
        for (index, entity) in selected_entities.iter().enumerate() {
            ...
            for text_id in entity.text_unit_ids.iter() {
                if text_id not in text_unit_ids_set and text_id in self.text_units {
                    let selected_unit = deepcopy(self.text_units[text_id]);
                    let num_relationships = count_relationships(
                        entity_relationships, selected_unit
                    );
                    text_unit_ids_set.add(text_id);
                    unit_info_list.push((selected_unit, index, num_relationships));
                }
            }
        }
        ...
        let selected_text_units = [unit[0] for unit in unit_info_list]
        ...
        if return_candidate_context {
            ...
            let context_key = context_name.lower();
            if context_key not in context_data {
    ...
    fn _build_local_context(
        ...
        let entity_tokens = num_tokens(entity_context, self.token_encoder);
        ...
        for entity in selected_entities {
            ...
            let total_tokens = entity_tokens + num_tokens(
                relationship_context, self.token_encoder
            );

            // build covariate context
            for covariate in self.covariates {
                let (covariate_context, covariate_context_data) = build_covariates_context(
                    selected_entities=added_entities,
                ...
                current_context.push(covariate_context);
                current_context_data[covariate.lower()] = covariate_context_data
            }

            if total_tokens > max_tokens {
                info!("Reached token limit - reverting to previous context state")
                break
         }
        ...
        let final_context_text = entity_context + "\n\n" + "\n\n".join(final_context);
        ...
        (final_context_text, final_context_data)
    }
}
