...
use crate::prompts::query::global_search_knowledge_system_prompt::GENERAL_KNOWLEDGE_INSTRUCTION;
use crate::prompts::query::global_search_map_system_prompt::MAP_SYSTEM_PROMPT;
use crate::prompts::query::global_search_reduce_system_prompt::{
    NO_DATA_ANSWER,
    REDUCE_SYSTEM_PROMPT,
};
use crate::query::context_builder::builders::GlobalContextBuilder;
use crate::query::context_builder::conversation_history::ConversationHistory;
use crate::query::llm::text_utils::{num_tokens, try_parse_json_object};
...
pub struct GlobalSearchResult(SearchResult) {
    map_responses: Vec<SearchResult>,
    reduce_context_data: str | Vec<LazyFrame> | dict[str, LazyFrame],
    reduce_context_text: str | Vec<str> | dict[str, str],
}

/// Search orchestration for global search mode.
pub struct GlobalSearch(BaseSearch[GlobalContextBuilder]) {
}
...
impl GlobalSearch {
    pub fn new(
        ...
        map_system_prompt: Option<String> /* = None */,
        reduce_system_prompt: Option<String> /* = None */,
        response_type: str = "multiple paragraphs",
        allow_general_knowledge: bool = False,
        general_knowledge_inclusion_prompt: Option<String> /* = None */,
        json_mode: bool = True,
        callbacks: Vec<QueryCallbacks> | None = None,
        max_data_tokens: int = 8000,
        map_llm_params: HashMap<String, Box<dyn Any>> = DEFAULT_MAP_LLM_PARAMS,
        reduce_llm_params: HashMap<String, Box<dyn Any>> = DEFAULT_REDUCE_LLM_PARAMS,
        ...
        **llm_kwargs,
        ...
        try:
            ...
            async with self.semaphore:
                let model_response = await self.model.achat(
                    prompt=query,
                ...
                info!("Map response: %s", search_response)
            try:
    ...
    def _parse_search_response(self, search_response: str) -> list[HashMap<String, Box<dyn Any>>]:
        ...
        **llm_kwargs,
        ...
        try:
            ...
            for point in filtered_key_points:
                ...
                formatted_response_data.push(
                    format!("----Analyst {point['analyst'] + 1}----")
                )
                formatted_response_data.push(
                    format!("Importance Score: {point['score']}")  # type: ignore
                )
                formatted_response_data.push(point["answer"])  # type: ignore
                ...
                if (
                    ...
                    break
                data.push(formatted_response_text)
    ...
    async def _stream_reduce_response(
        ...
        **llm_kwargs,
        ...
        for point in filtered_key_points:
            formatted_response_data = [
                format!("----Analyst {point['analyst'] + 1}----"),
                format!("Importance Score: {point['score']}"),
                point["answer"],
            ]
            formatted_response_text = "\n".join(formatted_response_data)
            if (
