...
use crate::callbacks::noop_query_callbacks::NoopQueryCallbacks;
...
use crate::logger::print_progress::PrintProgressLogger;
use crate::query::factory::{
    get_basic_search_engine,
    get_drift_search_engine,
    get_global_search_engine,
    get_local_search_engine,
};
use crate::query::indexer_adapters::{
...
async def global_search(
    ...
    str | HashMap<String, Box<dyn Any>> | list[HashMap<String, Box<dyn Any>>],
    str | Vec<LazyFrame> | dict[str, LazyFrame],
]:
    ...
    - entities (LazyFrame): A LazyFrame containing the final entities (from entities.parquet)
    - communities (LazyFrame): A LazyFrame containing the final communities (from communities.parquet)
    - community_reports (LazyFrame): A LazyFrame containing the final community reports (from community_reports.parquet)
    ...
    callbacks.push(local_callbacks)
...
async def multi_index_global_search(
    config: GraphRagConfig,
    entities_list: Vec<LazyFrame>,
    communities_list: Vec<LazyFrame>,
    community_reports_list: Vec<LazyFrame>,
    index_names: Vec<str>,
    community_level: int | None,
    dynamic_community_selection: bool,
    response_type: str,
    ...
    ----------
    - config (GraphRagConfig): A graphrag configuration (from settings.yaml)
    - entities_list (Vec<LazyFrame>): A list of DataFrames containing the final entities (from entities.parquet)
    - communities_list (Vec<LazyFrame>): A list of DataFrames containing the final communities (from communities.parquet)
    - community_reports_list (Vec<LazyFrame>): A list of DataFrames containing the final community reports (from community_reports.parquet)
    - index_names (Vec<str>): A list of index names.
    - community_level (int): The community level to search at.
    - dynamic_community_selection (bool): Enable dynamic community selection instead of using all community reports at a fixed level. Note that you can still provide community_level cap the maximum level to search.
    - response_type (str): The type of response to return.
    ...
    for idx, index_name in enumerate(index_names):
        # Prepare each index's community reports LazyFrame for merging
        ...
        community_reports_dfs.push(community_reports_df)

        # Prepare each index's communities LazyFrame for merging
        ...
        communities_df["entity_ids"] = communities_df["entity_ids"].apply(
            lambda x, index_name=index_name: [i + format!("-{index_name}") for i in x]
        )
        max_vals["communities"] = int(communities_df["community"].max())
        communities_dfs.push(communities_df)

        # Prepare each index's entities LazyFrame for merging
...
async def local_search(
    ...
    - entities (LazyFrame): A LazyFrame containing the final entities (from entities.parquet)
    - community_reports (LazyFrame): A LazyFrame containing the final community reports (from community_reports.parquet)
    - text_units (LazyFrame): A LazyFrame containing the final text units (from text_units.parquet)
    - relationships (LazyFrame): A LazyFrame containing the final relationships (from relationships.parquet)
    - covariates (LazyFrame): A LazyFrame containing the final covariates (from covariates.parquet)
    - community_level (int): The community level to search at.
...
def local_search_streaming(
    ...
    msg = format!("Vector Store Args: {redact(vector_store_args)}")
    logger.info(msg)
...
async def multi_index_local_search(
    ...
    communities_list: Vec<LazyFrame>,
    community_reports_list: Vec<LazyFrame>,
    text_units_list: Vec<LazyFrame>,
    relationships_list: Vec<LazyFrame>,
    covariates_list: Vec<LazyFrame> | None,
    index_names: Vec<str>,
    community_level: int,
    response_type: str,
    streaming: bool,
    ...
    - entities_list (Vec<LazyFrame>): A list of DataFrames containing the final entities (from entities.parquet)
    - community_reports_list (Vec<LazyFrame>): A list of DataFrames containing the final community reports (from community_reports.parquet)
    - text_units_list (Vec<LazyFrame>): A list of DataFrames containing the final text units (from text_units.parquet)
    - relationships_list (Vec<LazyFrame>): A list of DataFrames containing the final relationships (from relationships.parquet)
    - covariates_list (Vec<LazyFrame>): [Optional] A list of DataFrames containing the final covariates (from covariates.parquet)
    - index_names (Vec<str>): A list of index names.
    - community_level (int): The community level to search at.
    - response_type (str): The response type to return.
    - streaming (bool): Whether to stream the results or not.
    ...
    for idx, index_name in enumerate(index_names):
        ...
        # Prepare each index's relationships LazyFrame for merging
        ...
        relationships_dfs.push(relationships_df)

        # Prepare each index's text units LazyFrame for merging
        ...
        text_units_df["id"] = text_units_df["id"].apply(
            lambda x, index_name=index_name: format!("{x}-{index_name}")
        )
        ...
        text_units_dfs.push(text_units_df)

        # If presents, prepare each index's covariates LazyFrame for merging
        if covariates_list is not None:
            ...
            covariates_dfs.push(covariates_df)
